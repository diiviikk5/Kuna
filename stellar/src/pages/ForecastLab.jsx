...
const ForecastLab = () => {
    ...
    const CustomTooltip = ({ active, payload, label }) => {
        ...
        return (
            <div className="bg-[#020617] border border-white/[0.06] p-3 font-mono">
                <div className="text-[10px] text-slate-500 mb-2 border-b border-slate-800 pb-2 uppercase font-bold">
                    HORIZON_SYNC: +{label}
                </div>
                {payload.map((entry, index) => (
                    <div key={index} className="flex justify-between gap-6 py-0.5">
                        ...
                        <span className={`text-[10px] font-bold ${entry.name === 'Stellar' ? 'text-emerald-500' : 'text-indigo-400'}`}>
    ...
    return (
        <div className="min-h-screen">
            ...
            <div className="p-8 space-y-8">
                {/* Tab Navigation */}
                <div className="flex border border-white/[0.06] bg-slate-950/50 p-1">
                    {tabs.map((tab) => (
                        <button
                            key={tab.id}
                            onClick={() => setActiveTab(tab.id)}
                            className={`
                                relative flex items-center gap-3 px-8 py-4 font-semibold text-[10px] uppercase tracking-wider
                                transition-all
                                ${activeTab === tab.id
                                    ? 'bg-indigo-500 text-black shadow-lg shadow-indigo-500/10'
                                    : 'text-slate-500 hover:bg-[#0f172a]/80 hover:text-slate-300'
                                }
                            `}
                        >
                ...
                <AnimatePresence mode="wait">
                    {activeTab === 'architecture' && (
                        ...
                        >
                            ...
                            <div className="neo-panel bg-[#0f172a]/80">
                                <div className="flex items-center gap-4 mb-8">
                                    <h2 className="text-xl font-semibold uppercase tracking-wider text-white italic">TRANSFORMER_ENCODER_LSTM</h2>
                            ...
                            <div className="space-y-4">
                                {[
                                    ...
                                    <motion.div
                                        key={feature.title}
                                        className="neo-panel bg-[#020617] border-slate-800 p-6 flex gap-6 items-start hover:border-amber-500/30 group transition-all"
                                        initial={{ opacity: 0, x: 20 }}
                                        animate={{ opacity: 1, x: 0 }}
                                        transition={{ delay: i * 0.1 }}
                                    >
                                        <div className="w-12 h-12 bg-[#0f172a]/80 border border-white/[0.06] flex items-center justify-center text-indigo-400 shadow-lg shadow-indigo-500/10 group-hover:bg-indigo-500 group-hover:text-black transition-all">
                                            <feature.icon className="w-6 h-6" />
                                        </div>
                                        <div className="flex-1">
                                            <div className="flex items-center justify-between mb-2">
                                                <h4 className="text-white font-semibold uppercase text-sm tracking-wider">{feature.title}</h4>
                                                <span className="text-[11px] font-semibold text-slate-300 bg-[#0f172a]/80 px-2 py-0.5 border border-white/[0.06]">{feature.tag}</span>
                                            </div>
                    ...
                    {activeTab === 'training' && (
                        <motion.div
                            ...
                            animate={{ opacity: 1 }}
                            exit={{ opacity: 0 }}
                            className="neo-panel bg-[#0f172a]/80 border border-white/[0.06] p-8"
                        >
                            <div className="flex items-center gap-4 mb-8">
                                <h2 className="text-xl font-semibold uppercase tracking-wider text-white italic">ML_TRAINING_GROUND</h2>
                                <div className="flex-1 h-[1px] bg-slate-800" />
                                
                                {/* Orbit Type Selector */}
                                <div className="flex items-center gap-2">
                                    <span className="text-[10px] font-semibold text-slate-500 uppercase">ORBIT_TYPE:</span>
                                    <div className="flex gap-2">
                                        {['GEO', 'MEO'].map((orbit) => (
                                            <button
                                                key={orbit}
                                                onClick={() => setSelectedOrbit(orbit)}
                                                className={`px-4 py-2 text-[10px] font-semibold uppercase tracking-wider border-2 transition-all ${
                                                    selectedOrbit === orbit
                                                        ? 'bg-indigo-400 border-amber-400 text-black'
                            ...
                            <div className="grid grid-cols-1 gap-12">
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 pt-8 border-t border-white/[0.06]">
                                    <div className="space-y-6">
                                        <h4 className="text-xs font-semibold text-slate-300 uppercase tracking-wider mb-4 border-l-2 border-indigo-500 pl-4">DATA_PIPELINE_FLOW</h4>
                                        {[
                                            ...
                                            <div key={i} className="flex items-center gap-6 p-4 bg-[#020617] border border-slate-800 font-mono">
                                                <div className="w-10 h-10 border border-white/[0.06] flex items-center justify-center text-xs font-bold text-slate-400">{item.step}</div>
                                                <div>
                                                    <div className="text-xs text-slate-100 font-bold uppercase tracking-wider">{item.title}</div>
                                    ...
                                    <div className="space-y-6">
                                        <h4 className="text-xs font-semibold text-slate-300 uppercase tracking-wider mb-4 border-l-4 border-slate-700 pl-4">VALIDATION_METRICS</h4>
                                        <div className="grid grid-cols-2 gap-4">
                                            {[
                                                ...
                                                <div key={i} className="neo-panel bg-[#020617] border-slate-800 p-6 shadow-none text-center">
                                                    <div className="text-4xl font-semibold text-slate-100 italic">{s.val}</div>
                                                    <div className="text-[10px] font-semibold text-slate-500 mt-2 uppercase tracking-wider">{s.label}</div>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                    ...
                    {activeTab === 'evaluation' && (
                        ...
                        >
                            <div className="mb-6">
                                <h2 className="text-xl font-semibold uppercase tracking-wider text-white italic">ISRO_EVALUATION_DASHBOARD</h2>
                            ...
                            {evaluationData ? (
                                ...
                                <div className="neo-panel bg-[#0f172a]/80 border border-white/[0.06] p-12 text-center">
                                    <BeakerIcon className="w-16 h-16 mx-auto text-slate-700 mb-6" />
                                    <h3 className="text-lg font-semibold text-slate-400 uppercase mb-2">
                                        No Evaluation Data
                                    </h3>
                                    ...
                                    <button
                                        ...
                                        className="mt-6 px-6 py-3 bg-indigo-400 text-black font-semibold text-xs uppercase tracking-wider hover:bg-amber-400 transition-colors"
                                    >
                                        Go to Training
                                    </button>
                                </div>
                        ...
                        >
                            ...
                            <div className="neo-panel bg-[#0f172a]/80">
                                <div className="flex items-center justify-between mb-12 flex-wrap gap-4">
                                    <div>
                                        <h3 className="text-xl font-semibold text-white uppercase tracking-wider italic">MULTI_HORIZON_ERROR_DYNAMICS</h3>
                                        <p className="text-[11px] text-slate-300 font-bold uppercase mt-1 tracking-wider">Global RMSE validation across 8 temporal windows</p>
                                    </div>
                                    <HorizonSelector
                                    ...
                                    <ResponsiveContainer width="100%" height={400}>
                                        <BarChart data={comparisonData} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                                            ...
                                            <Legend
                                                ...
                                                formatter={(value) => <span className="text-slate-300 text-[11px] uppercase font-bold font-mono tracking-wider">{value}</span>}
                                            />
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                                {comparisonData.slice(0, 4).map((item, i) => (
                                    <motion.div
                                        key={item.horizon}
                                        className={`neo-panel bg-[#020617] border-slate-800 p-8 text-center transition-all ${item.horizon === selectedHorizon ? 'border-indigo-500/30 shadow-[8px_8px_0px_#000]' : 'opacity-60 grayscale'}`}
                                        initial={{ opacity: 0, y: 20 }}
                                        animate={{ opacity: 1, y: 0 }}
                                        transition={{ delay: i * 0.1 }}
                                    >
                                        <div className="text-[11px] text-slate-300 font-semibold uppercase tracking-wider mb-4">@{item.horizon}_WINDOW</div>
                                        <div className="text-5xl font-semibold text-emerald-500 leading-none mb-2 italic">
                                            -{Math.round((1 - item.Stellar / item.Baseline) * 100)}%
                                        </div>
                                        <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">RMSE_REDUCTION</div>
                                    </motion.div>
                                ))}
                            </div>
                        </motion.div>
                ...
                {/* Evidence Strip */}
                <div className="neo-panel bg-[#020617] border-slate-800 p-2 text-white">
                    <EvidenceStrip
